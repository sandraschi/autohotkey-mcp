...
async def api_llm_settings() -> JSONResponse:
    ...
    provider = "ollama"
    if "1234" in base or "lmstudio" in base.lower():
        provider = "lmstudio"
...
async def api_llm_settings_put(request: Request) -> JSONResponse:
    ...
    if "model" in body:
        ...
        if m:
            _llm_runtime["model"] = m
            updated["model"] = m
    if "api_key" in body:
        k = str(body["api_key"]).strip()
        _llm_runtime["api_key"] = k
        updated["api_key_set"] = bool(k)
    if "timeout" in body:
        try:
            t = float(body["timeout"])
            _llm_runtime["timeout"] = t
            updated["timeout"] = t
    ...
    if "model" in updated:
        _llm_mod.DEFAULT_MODEL = updated["model"]
    if "timeout" in updated:
        _llm_mod.DEFAULT_TIMEOUT = updated["timeout"]
...
async def api_llm_models() -> JSONResponse:
    ...
    async with httpx.AsyncClient(timeout=8.0) as client:
        ...
        try:
            # strip /v1 suffix if present to get Ollama root
            ollama_base = base.rstrip("/v1").rstrip("/")
            if not ollama_base.endswith("/v1"):
                r = await client.get(f"{ollama_base}/api/tags", headers=headers)
        ...
        # OpenAI-compat: GET {base}/models  →  {"data": [{"id": "..."}, ...]}
        if not models:
            try:
...
async def api_run_scriptlet(request: Request) -> JSONResponse:
    """SPA: run a scriptlet by id."""
    try:
        body = await request.json()
    except Exception:
        return JSONResponse(status_code=400, content={"success": False, "error": "Invalid JSON"})
