...
```powershell
git clone https://github.com/sandraschi/autohotkey-mcp
cd autohotkey-mcp
just
```
...
4. Start the server:
   ...
   uv run python -m autohotkey_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn autohotkey_mcp.server:app --port 10746
   ```
5. Open `http://localhost:10746` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/autohotkey-mcp/issues) |
